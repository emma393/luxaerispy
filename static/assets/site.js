...
function luxaerisInjectRail(){
  ...
  // remove duplicate floating buttons or duplicated request cards outside container
  document.querySelectorAll('a.floating-request, .popup-overlay, .funnel-banner').forEach(el => el.remove());

  let rail = container.querySelector(':scope > .luxaeris-fixed-rail');
  if (!rail) {
    container.insertAdjacentHTML('beforeend', luxaerisRailHTML(contextTitle));
  ...
  if (!mainCol) {
    ...
    container.insertBefore(mainCol, rail || null);
  }

  // flatten accidental nested wrappers
  let nested = mainCol.querySelector(':scope > .luxaeris-main-column');
  while (nested && mainCol.children.length === 1) {
    const frag = document.createDocumentFragment();
    while (nested.firstChild) frag.appendChild(nested.firstChild);
    nested.remove();
    mainCol.appendChild(frag);
    nested = mainCol.querySelector(':scope > .luxaeris-main-column');
  }

  // if existing rail ended up inside main column, move it out
  mainCol.querySelectorAll(':scope .luxaeris-fixed-rail').forEach(innerRail => {
    if (innerRail !== rail) innerRail.remove();
  });

  // ensure left content/right rail ordering
  if (rail && rail.parentElement !== container) container.appendChild(rail);
  if (rail && rail.previousElementSibling !== mainCol) container.appendChild(rail);
