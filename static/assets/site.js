...
\nfunction buildLuxAerisRail(){return `<aside class="luxaeris-request-rail"><div class="sticky-card"><p class="kicker">Request tailored options</p><h2>Get premium flight options without leaving this page.</h2><p>Share your route, dates, and cabin preference to request business class, first class, or premium economy options through LuxAeris. LuxAeris does not sell tickets directly, and there is no LuxAeris service fee for submitting a request.</p><ul class="rail-list"><li>No LuxAeris booking fee</li><li>Business, first, and premium economy</li><li>Visible while you browse the page</li></ul><form class="sticky-quote-form" data-sticky-request="1" novalidate><div class="trip-switch"><button type="button" class="is-active" data-trip="Round Trip">Round Trip</button><button type="button" data-trip="One Way">One Way</button><button type="button" data-trip="Multi City">Multi City</button></div><input type="hidden" name="tripType" value="Round Trip"><div class="full"><input name="origin" data-location-input placeholder="Origin — city or airport" required></div><div class="full"><input name="destination" data-location-input placeholder="Destination — city or airport" required></div><div><input type="date" name="departDate" required></div><div class="return-wrap"><input type="date" name="returnDate" required></div><div><select name="cabin"><option>Business Class</option><option>First Class</option><option>Premium Economy</option></select></div><div><input name="fullName" placeholder="Full name" required></div><div class="full"><input name="email" type="email" placeholder="Email" required></div><div class="full"><input name="phone" type="tel" placeholder="Phone or WhatsApp" required></div><div class="full"><textarea name="notes" placeholder="Preferred airline, schedule, passengers, or anything helpful"></textarea></div><div class="microcopy">Not sure about exact dates or airports yet? Submit the closest option and refine it after the request is received.</div><div class="status" aria-live="polite"></div><div class="submit-row"><button type="submit" class="btn btn-primary">Submit request</button></div></form></div></aside>`}function shouldAddLuxAerisRail(){const p=window.location.pathname;if(p==='/'||p.endsWith('/index.html')||p==='/request.html'||p==='/about.html'||p==='/contact.html'||p==='/privacy-policy.html'||p==='/terms-and-conditions.html'||p==='/cookie-policy.html'||p==='/disclaimer.html'||p==='/thank-you.html'||p==='/search.html'||p==='/sitemap-page.html')return false;return /\/(routes|destinations|airports|airlines|cabins)\//.test(p);}function addLuxAerisRail(){if(!shouldAddLuxAerisRail()||document.querySelector('.luxaeris-request-rail'))return;document.body.classList.add('has-sticky-rail');document.querySelectorAll('.floating-request').forEach(el=>el.remove());const c=document.querySelector('.page-shell > .container');if(c){const main=document.createElement('div');main.className='luxaeris-main-column';while(c.firstChild)main.appendChild(c.firstChild);c.appendChild(main);c.insertAdjacentHTML('beforeend',buildLuxAerisRail());return;}const fg=document.querySelector('.feature-grid');if(fg){fg.insertAdjacentHTML('beforeend',buildLuxAerisRail());}}function bindStickyRailForms(){document.querySelectorAll('[data-sticky-request]').forEach(form=>{if(form.dataset.bound==='1')return;form.dataset.bound='1';form.querySelectorAll('[data-trip]').forEach(btn=>{btn.addEventListener('click',()=>{form.querySelectorAll('[data-trip]').forEach(b=>b.classList.toggle('is-active',b===btn));const type=btn.dataset.trip||'Round Trip';form.querySelector('[name="tripType"]').value=type;const wrap=form.querySelector('.return-wrap');const input=form.querySelector('[name="returnDate"]');const need=type==='Round Trip';if(wrap)wrap.style.display=need?'':'none';if(input){input.required=need;if(!need)input.value='';}})});form.querySelectorAll('[data-location-input]').forEach(attachAirportAutocomplete);form.addEventListener('submit',async e=>{e.preventDefault();let ok=true;form.querySelectorAll('[data-location-input]').forEach(input=>{const valid=markAirportFieldValidity(input);input.classList.toggle('field-invalid',!valid);ok=valid&&ok;});const fullName=form.querySelector('[name="fullName"]');const email=form.querySelector('[name="email"]');const phone=form.querySelector('[name="phone"]');if(fullName){const good=validName(fullName.value);fullName.setCustomValidity(good?'':'Please enter your full name.');ok=good&&ok;}if(email){const good=validEmail(email.value);email.setCustomValidity(good?'':'Please enter a valid email address.');ok=good&&ok;}if(phone){const good=validPhone(phone.value);phone.setCustomValidity(good?'':'Please enter a valid phone number.');ok=good&&ok;}const status=form.querySelector('.status');if(!ok){if(status)status.textContent='Please complete the required fields and choose both airports from the dropdown list.';form.reportValidity();return;}const origin=getAirportRecordFromInput(form.querySelector('[name="origin"]'));const destination=getAirportRecordFromInput(form.querySelector('[name="destination"]'));if(!origin||!destination){if(status)status.textContent='Please choose both airports from the suggested list.';return;}const endpoint=(window.LUXAERIS_FORM_ENDPOINT||'').trim();if(!endpoint){if(status)status.textContent='Add your endpoint in assets/site-config.js to activate submissions.';return;}const payload=Object.fromEntries(new FormData(form).entries());payload.originCode=origin.code;payload.destinationCode=destination.code;payload.originCity=origin.city||'';payload.destinationCity=destination.city||'';payload.routeType=inferRouteType(origin.code,destination.code);payload.leadSource=window.location.pathname;const submit=form.querySelector('button[type="submit"]');if(submit)submit.disabled=true;if(status)status.textContent='Submitting your request…';try{const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!res.ok)throw new Error('bad');window.location.href='/thank-you.html';}catch(err){if(status)status.textContent='We could not submit your request just now. Please try again in a moment.';}finally{if(submit)submit.disabled=false;}});});}document.addEventListener('DOMContentLoaded',()=>{addLuxAerisRail();bindStickyRailForms();},{once:true});\n