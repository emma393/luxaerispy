...
class LuxAerisBuilder:
    ...
    def build_airports(self):
        ...
        for a in self.airports:
            ...
            sections = [
                {"title": f"{name} overview", "text": a["premium_summary"], "links": []},
            ...
            write_page(self.output_root, f"airports/{slug}.html", self.ctx(f"{code} Airport Lounge and Premium Travel Guide | {self.config['site_name']}", f"Premium travel guide to {name}. Discover lounges, airlines, airport flow, and premium route context with LuxAeris.", f"airports/{slug}.html", f"{name} Airport Guide", a["premium_summary"], a.get("featured_image", self.config["default_image"]), sections, related))
    ...
    def build_routes(self):
        ...
        for r in self.routes:
            ...
            related = [{"href": f"/airports/{r['origin_airport_slug']}.html", "label": f"{r['origin_airport_slug'].upper()} airport"},{"href": f"/airports/{r['destination_airport_slug']}.html", "label": f"{r['destination_airport_slug'].upper()} airport"}]
            sections = [{"title": "Route overview", "text": r["route_summary"], "links": []},{"title": "Airport context", "text": "These airport guides help you compare lounge quality, departure flow, and arrival practicality.", "links": related}]
            write_page(self.output_root, f"routes/{slug}.html", self.ctx(f"{name} | {self.config['site_name']}", f"Compare {cabin.lower()} flights from {origin} to {dest}. Explore premium airlines, airport flow, and request a tailored fare quote.", f"routes/{slug}.html", name, r["route_summary"], r.get("featured_image", self.config["default_image"]), sections, related))
            items.append({"slug": slug, "name": name})
    ...
    def build_lounges(self):
        ...
        for l in self.lounges:
            ...
            sections = [{"title": "Lounge overview", "text": l["customer_summary"], "links": []},{"title": "Useful lounge features", "text": "The most valuable premium lounges usually improve the pre-flight experience through better comfort, food, and practical amenities.", "links": feats},{"title": "Airport context", "text": "Open the airport guide to compare this lounge with broader airport flow and route context.", "links": related}]
            write_page(self.output_root, f"lounges/{slug}.html", self.ctx(f"{name} | {self.config['site_name']}", f"Guide to {name}. Explore terminal location, access context, and premium travel detail with LuxAeris.", f"lounges/{slug}.html", name, l["customer_summary"], l.get("featured_image", self.config["default_image"]), sections, related))
            items.append({"slug": slug, "name": name})
    ...
    def build_airlines(self):
        ...
        for a in self.airlines:
            ...
            sections = [{"title": "Airline cabin overview", "text": a["premium_summary"], "links": []},{"title": "Related aircraft", "text": "Aircraft type influences privacy, seat layout, and the overall premium feel of the journey.", "links": related}]
            write_page(self.output_root, f"airlines/{slug}.html", self.ctx(f"{name} Review | {self.config['site_name']}", f"Compare {name.lower()}, route fit, aircraft, and premium travel context before requesting a tailored fare quote.", f"airlines/{slug}.html", name, a["premium_summary"], a.get("featured_image", self.config["default_image"]), sections, related))
            items.append({"slug": slug, "name": name})
    ...
    def build_aircraft(self):
        ...
        for a in self.aircraft:
            ...
            sections = [{"title": "Aircraft overview", "text": a["premium_summary"], "links": []},{"title": "Airlines using this aircraft style", "text": "Different airlines configure the same aircraft very differently, which is why aircraft and airline should be judged together.", "links": related}]
            write_page(self.output_root, f"aircraft/{slug}.html", self.ctx(f"{name} | {self.config['site_name']}", f"Explore {name.lower()} cabins, airline use, and premium travel context with LuxAeris.", f"aircraft/{slug}.html", name, a["premium_summary"], a.get("featured_image", self.config["default_image"]), sections, related))
            items.append({"slug": slug, "name": name})
    ...
    def build_destinations(self):
        ...
        for d in self.destinations:
            ...
            sections = [{"title": "Destination overview", "text": d["luxury_summary"], "links": []},{"title": "Relevant premium routes", "text": "These route guides connect the destination with practical premium flight options.", "links": related}]
            write_page(self.output_root, f"destinations/{slug}.html", self.ctx(f"Business Class Flights to {name} | {self.config['site_name']}", f"Compare luxury business class and first class flights to {name}. Explore premium routes and request a tailored fare quote with LuxAeris.", f"destinations/{slug}.html", f"Flights to {name}", d["luxury_summary"], d.get("featured_image", self.config["default_image"]), sections, related))
            items.append({"slug": slug, "name": name})
    ...
    def build_flights(self):
        ...
        for f in self.flights:
            ...
            sections = [{"title": "Flight overview", "text": f["flight_summary"], "links": []},{"title": "Route context", "text": "Flight-number pages help connect a specific premium flight with its route, airport quality, and broader planning context.", "links": related}]
            write_page(self.output_root, f"flight/{slug}.html", self.ctx(f"{number} Flight Guide | {self.config['site_name']}", f"Explore {number} route context, airport fit, and premium travel planning insight with LuxAeris.", f"flight/{slug}.html", number, f["flight_summary"], f.get("featured_image", self.config["default_image"]), sections, related))
