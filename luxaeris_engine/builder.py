...
<header class="site-header">
  <div class="container nav">
    <a class="logo-wrap" href="/index.html">
      ...
      <div><div class="brand-name">{self.config["site_name"]}</div><div class="brand-tag">{self.config["brand_tagline"]}</div></div>
    </a>
  </div>
</header>
<section class="section">
  <div class="container">
    ...
    <div class="index-visual-grid">{cards}</div>
  </div>
</section>
</body>
</html>"""
            ...
            items.append(
                {
                    "slug": slug,
                    "name": name,
                    "img": a.get("featured_image", self.config["default_image"]),
                    "desc": a.get("premium_summary", "")[:150],
                }
            )
        self.build_index(
            "airports",
    ...
    def build_routes(self):
        ...
        for r in self.routes:
            ...
            sections = [
                ...
                {"title": "Airport context", "text": "These airport guides help you compare lounge quality, departure flow, and arrival practicality.", "links": related},
            ]
            write_page(
                self.output_root,
                f"routes/{slug}.html",
            ...
            items.append(
                {
                    "slug": slug,
                    "name": name,
                    "img": r.get("featured_image", self.config["default_image"]),
                    "desc": r.get("route_summary", "")[:150],
                }
            )
        self.build_index(
            "routes",
            ...
            items.append(
                {
                    "slug": slug,
                    "name": name,
                    "img": l.get("featured_image", self.config["default_image"]),
                    "desc": l.get("customer_summary", "")[:150],
                }
            )
        self.build_index(
            "lounges",
    ...
    def build_destinations(self):
        ...
        for d in self.destinations:
            ...
            sections = [
                {"title": "Destination overview", "text": d.get("luxury_summary", ""), "links": []},
                {"title": "Relevant premium routes", "text": "These route guides connect the destination with practical premium flight options.", "links": related},
            ]
            write_page(
                self.output_root,
            ...
            items.append(
                {
                    "slug": slug,
                    "name": name,
                    "img": d.get("featured_image", self.config["default_image"]),
                    "desc": d.get("luxury_summary", "")[:150],
                }
            )
        self.build_index(
            "destinations",
            ...
            items.append(
                {
                    "slug": slug,
                    "name": number,
                    "img": f.get("featured_image", self.config["default_image"]),
                    "desc": f.get("flight_summary", "")[:150],
                }
            )
        self.build_index(
            "flight",
